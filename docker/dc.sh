...
COMPOSE_OVERRIDE=
PHP_CONTAINER=php_broadway_sensitive_serializer_dbal
PHP_USER=utente
...
DC_EXEC_NO_PSEUDO_TTY="${DC_BASE_COMMAND}
  exec
  -T
  -u ${PHP_USER}
  -w ${WORKDIR}
...
elif [[ "$1" == "enter" ]]; then                                        ${DC_EXEC} /bin/zsh
...
elif [[ "$1" == "coding-standard-fix-no-pseudo-tty" ]]; then shift 1;   ${DC_EXEC_NO_PSEUDO_TTY} $PROJECT_TOOL coding-standard-fix "$@"
...
elif [[ "$1" == "psalm-no-pseudo-tty" ]]; then shift 1;                 ${DC_EXEC_NO_PSEUDO_TTY} $PROJECT_TOOL psalm "$@"
